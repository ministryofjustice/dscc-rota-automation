...
public class MainWindowController implements Initializable {
    ...
    private ObservableList<TableModel> tableData() {
        ...
        for (int i = 0; i < columns.size(); i++) {
            tableRows.add(new TableModel(this.columns.get(i).getTitle(), this.columns.get(i).getType(), this.columns.get(i).getSchemes(), this.columns.get(i).getChosenModifiers()));
        }
        return tableRows;
    }
    ...
    public Stage showColumnDialog(LocalDate start, LocalDate end, ArrayList<Scheme> schemes) throws IOException {
    ...
    /**
     * Enables and disables date selectors and output selection elements
    ...
    public void generateClicked(ActionEvent actionEvent) {
        try {
           System.out.println(this.columns.get(0).getTitle());
            RotaManager manager = new RotaManager(importedDataset, dateStart.getValue(), dateEnd.getValue(), this.columns);
     ...
     * @return A scheme object corresponding to the specified name, or null
    ...
    public void gatherColumnData(ColumnType type, String title, ArrayList<Scheme> schemes, int[] blanksPattern, ArrayList<BlanksModifier> blanksModifiers) {
