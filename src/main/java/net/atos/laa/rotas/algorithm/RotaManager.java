...
public class RotaManager {
    ...
    private ArrayList<ColumnTemplate> columnInfo;
    ...
    public RotaManager(
        Dataset dataset, LocalDate startDate, LocalDate endDate, ArrayList<ColumnTemplate> columnInfo) {
...
//        }
    ...
    /**
     * Clears the allocations list and adds a zero value for every solicitor and calls the resetSchemeSets to reset them
     */
...
//                    System.out.println("Col loop");
                    ...
                    if (schemes.get(columnTemplate.getSchemes()) != schemeSet) continue;
                    ...
                    if (slots[row].getSolicitor() == null && !slots[row].getIsBlank()) {
                        ...
                        if (!groupOfSolicitors(row, slots.length - 1,
                                workingRota.getColumnList().size()).contains(peekSolicitor(columnTemplate).getName())) {
    ...
    /**
     * Gets the solicitor currently being worked on, increments the number
     * of times that solicitor has been allocated, then moves to the next solicitor
     *
    ...
    private Solicitor getSolicitor(ColumnTemplate col, int row) {
        ...
        changeDaysAlloc(dayAllocations, solicitor, workingRota.getStartDate().plusDays(row).getDayOfWeek().getValue(), +1);
        return solicitor;
    }
    ...
    /**
     * The groups the solicitors of the current row, aswell as the row before and after,
     * into one arraylist to be used as a comparison
     *
     ...
     * @return Returns an arraylist containing the names of all solicitors in the row above, below
     * and the same row specified
     */
    ...
    /**
     * This calls redistribute row on every row in the rota in an attempt to even out the allocations of types
     */
    ...
    /**
     * This redistributes each row to even out type allocation. This happens by checking along the row and comparing each solicitior to see if one has more of a type of allocation than the other
    ...
    private void redistributeRow(int row) {
        ...
        for (int col = 0; col < workingRota.getColumnList().size(); col++) {
            column = workingRota.getColumnList().get(col);
            if (column.getSlots()[row].getSolicitor() == null) continue;
            ...
            for(int i = col; i<workingRota.getColumnList().size();i++){
                ...
                if(column.getType()==tempColumn.getType()||(schemes.get(columnInfo.get(col).getSchemes())!=schemes.get(columnInfo.get(i).getSchemes()))||tempColumn.getSlots()[row].getSolicitor()==null){
                    continue;
                }
                Solicitor solicitor = tempColumn.getSlots()[row].getSolicitor();
                if(alloc.get(sol.getSolicitorId())>alloc.get(solicitor.getSolicitorId())&&oppositeAlloc.get(sol.getSolicitorId())<oppositeAlloc.get(solicitor.getSolicitorId())){
                    System.out.println("Switching Sol: "+sol.getName()+", "+alloc.get(sol.getSolicitorId())+" with Sol: " + solicitor.getName() + ", " + alloc.get(solicitor.getSolicitorId()));
     ...
     */
    private void changeDaysAlloc(HashMap<Integer, ArrayList<Integer>> daysAlloc, Solicitor sol, int day, int change) {
