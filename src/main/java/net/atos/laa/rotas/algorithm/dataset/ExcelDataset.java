...
/**
 * Manages access to a single source of rota data in Excel format. Enables access to the list of
...
public class ExcelDataset implements Dataset {
  ...
  /**
   * Creates and opens a connection to a dataset for accessing data in the Excel format
   *
  ...
  /**
   * Goes through the dataset and produces an in-memory relationable representation of the data
   */
    ...
    for (Point startCell : startCells) {
      ...
      while (row != null) {
        if (dataFormatter.formatCellValue(row.getCell(
            startCell.x)).equals("Lead Office Account Number")) {
          break;
        }
  ...
  /**
   * Searches the Excel sheet for all cells indicating the start of a dataset fragment
   */
   ...
   * @return Returns a <code>Scheme</code> containing data for the specified scheme, or null if not
   * found
   */
