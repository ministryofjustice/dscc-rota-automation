...
public class RotaStats {
  ...
  private final HashMap<ArrayList<Scheme>, SchemeSets> schemeSets;
...
//    }
    ...
    for (int scheme : schemeIDs) {
      ...
      for (Solicitor sol : solicitors) {
        if (sol.getSchemeId() == scheme) {
          ...
          System.out
              .printf("%-35s Scheme: %d Total: %d Court: %d Police: %d DaysOfWeek: ", sol.getName(),
                  sol.getSchemeId(), temp, courtAllocations.get(sol.getSolicitorId()),
                  policeAllocations.get(sol.getSolicitorId()));
          ArrayList<Integer> days = dayAllocations.get(sol.getSolicitorId());
          System.out
              .printf("Mon: %-5d Tue: %-5d Wed: %-5d Thur: %-5d Fri %-5d Sat: %-5d Sun: %-5d \n",
                  days.get(0), days.get(1), days.get(2), days.get(3), days.get(4), days.get(5),
                  days.get(6));
  ...
  public float evaluateRota(ArrayList<RotaColumn> columnList) {
    ...
    fitness = clashes * clashWeighting + totalRange * rangeWeighting
        + rangeAverage * rangeAverageWeighting
  ...
  public void printStats() {
    ...
    System.out
        .println("Largest TotalRange = " + totalRange + " Average TotalRange = " + rangeAverage);
    System.out.println(
        "Largest PoliceRange = " + policeRange + " Average PoliceRange = " + policeAverage);
    System.out
        .println("Largest CourtRange = " + courtRange + " Average CourtRange = " + courtAverage);
    System.out
        .println("Largest DayRange = " + daysRange + " Average DayRange = " + daysRangeAverage);
  ...
  public void setTotalAllocations(HashMap<Integer, Integer> totalAllocations) {
    this.totalAllocations = totalAllocations;
  }

  public void setCourtAllocations(HashMap<Integer, Integer> courtAllocations) {
    this.courtAllocations = courtAllocations;
  }

  public void setPoliceAllocations(HashMap<Integer, Integer> policeAllocations) {
    this.policeAllocations = policeAllocations;
  }

  public void setDayAllocations(HashMap<Integer, ArrayList<Integer>> dayAllocations) {
    this.dayAllocations = dayAllocations;
  }

  public void setSchemeIDs(ArrayList<Integer> schemeIDs) {
    this.schemeIDs = schemeIDs;
  }

  public float getRangeAverage() {
    return rangeAverage;
  }
  ...
  public float getDaysRangeAverage() {
    return daysRangeAverage;
  }
  ...
  public float getCourtAverage() {
    return courtAverage;
  }

  public float getPoliceAverage() {
    return policeAverage;
  }

  public int getCourtRange() {
    return courtRange;
  }

  public int getPoliceRange() {
    return policeRange;
  }

  public int getColSeperation() {
    return colSeperation;
  }
}
